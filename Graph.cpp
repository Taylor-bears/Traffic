...
graph::graph(const string& file_path1, const string& file_path2, const string& file_path3) {
	mycity = city(file_path1, file_path2);//这里需要赋值
...
//最省钱有type限制有time限制路径 Money_Dijkstra_limit
...
//最省钱有time限制直达路径 findBestPath2_limit
...
void graph::find_twocity() {
	...
	cout << "两个城市间的直线距离为：" << mycity.distance_map[make_pair(city1, city2)] << "km" << endl;
	if (mycity.dist[i][j] != 0) {
		cout << "两个城市间的最短连通距离为：" << mycity.dist[i][j] << "km" << endl;
	}
