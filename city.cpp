...
//补充头文件中的city构造函数
city::city(const string& file_path, const string& file_path2) {
    ...
    if (input.is_open()) { //确保文件要被打开
        ...
        ifstream input2(file_path2);
        if (input2.is_open()) { //确保文件要被打开
            char nosense;//用于保存无用的符号
            string city_name1;
            string city_name2;
            ...
            while (input >> nosense) {
                if (nosense == '(') {
                    ...
                    //将城市与城市组合，并与距离构成哈希表
                    pair<string, string> distances = make_pair(city_name1, city_name2);//以两个城市作为一组               
                    this->link_map[distances] = distance_num; //不同城市之间的距离已形成哈希表
            ...
            for (int i = 0; i < number; i++) {
                for (int j = 0; j < number; j++) {
                    if (i == j) {
                        ...
                        auto it = link_map.find({ name[i], name[j] });
                        if (it != link_map.end()) {
                            // 设置A到B的距离
                            dist[i][j] = it->second;
                            // 设置B到A的距离，与A到B相等
